(function(){
	require([config.configUrl],function(){
		var reqArr = ['wxshare','swiper'];
		require(reqArr, requireCb);
	});
	
	function requireCb(wxshare,Swiper){
		//防止输入时，输入框弹出导致页面样式混乱
		var view_h = document.documentElement.clientHeight || document.body.clientHeight;
		$("body").height(view_h);

		var viewport = new Vue({
			el: '#queryListPage',
			data: {
				params: {
					channel: 'customer',
					product_id: localStorage.product_id,
					//查询所有信息，下面的参数暂时不传
					status: null,
					searchtype: null,
					keyword: null
					//token: localStorage.token  //用户端不需要
				},
				pageData: JSON.parse(localStorage.list)
			},
			methods: {
				init: function(){
					$('#queryListPage').show();
					M.loadingHide();
				},
				historyHandle: function(){
					window.history.go(-1);
				},
				homeHandle: function(){
					goToNextPage('index.html');
				},
				detailHandle: function(event, apply_no, index){
					var me = this;
					var target = event.currentTarget;
					//请求后台，获取详情
					M.loading();
					requstObj.orderdetail({
						channel: "customer",
						apply_no: apply_no
					}, function(data){
						console.log(data);
						var curr = me.pageData[index];
						curr.workflow = data.order.workflow.reverse();
						!curr.workflow.length && $(target).parents('.bigBox').find('.hint').show();
						Vue.set(me.pageData, index, curr);
						$(target).hide().siblings().show();
						$(target).parents('.bigBox').find('.stateBox').show();
						M.loadingHide();
					});
				},
				closeHandle: function(event){
					var target = event.currentTarget;
					$(target).hide().siblings().show();
					$(target).parents('.bigBox').find('.stateBox').hide();
					$(target).parents('.bigBox').find('.hint').hide();
				},
				backHandle: function(){
					goToNextPage('index.html');
				}
			}
		});

		var requstObj = {
			orderdetail: function(obj, cb){
				post('loan/common/orderdetail', obj, function(data){
					cb && cb(data);
				})
			}
		};
		
		function defaultError(data){
			var err = data.error - 0;
			M.loadingHide();
			switch(err){
				case 1002:
					oAuth.clear();
					M.alert('你的身份信息已过期，点击确定刷新页面');
					window.location.reload();
					break;
				default:
					M.alert(data.error_msg);
			}
		}
		
		function post(action,param,cb){
			M.ajax(action,param,config.gameid,function(data){
				var err = data.error - 0;
				switch(err){
					case 0:
						cb && cb(data);
						break;
					default:
						defaultError(data);
				}
			},config.apiopenid,config.apitoken,config.isDebug?'nf':'');
		}
		
		function share(shareInfo,succCb){
			wxshare.initWx(shareInfo,config.gameid,config.apiopenid,config.apitoken,succCb,null,null,null);
		}
		
		//产生随机数 例如，生成0-9的随机数(包括0,不包括9) random(0,9)
	    function random(min,max){
	    	return Math.floor(min+Math.random()*(max-min));
	    }

		M.loading();
		check(oAuth, function(){
			viewport.init();
		});
	}
}());
