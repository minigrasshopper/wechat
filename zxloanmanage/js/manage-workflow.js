(function(){
	...
	function requireCb(wxshare){
        ...
        var viewport = new Vue({
        	el: '#manageWorkflowPage',
        	data: {
				pageData: JSON.parse(localStorage.workflow).reverse()
        	},
        	methods: {
        		init: function(){
					console.log(this.pageData);
        			$('#manageWorkflowPage').show();
        			M.loadingHide();
        		},
				...
				goManageIssue: function(issues){
					localStorage.issues = JSON.stringify(issues);
					goToNextPage('manage-issue.html');
				}
        	}
        });
